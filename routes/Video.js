...
// Updating views
router.get("/videos/view/:id", async (req,res)=>{

    try{
    await VideoModel.findByIdAndUpdate(req.params.id, {$inc: {views: 1}}, { new: true })
     res.send("view updated")
    }catch(err){console.log(err)}
 })
// Updating likes
router.get("/videos/like/:id", async (req,res)=>{

    try{
    await VideoModel.findByIdAndUpdate(req.params.id, {$inc: {likes: 1}}, { new: true })
     res.send("like added")
    }catch(err){console.log(err)}
 })
